...
// Run: node --test test/events.model.test.js
const { suite, scratchDb } = require('./lib/suite');

suite('the events model', async ({ ok, t }) => {
  const dbm = require('../database/db');
  dbm.initDB(scratchDb(t, 'events.model'));
  const db = dbm.getDB();
  ...
  try { M.signUp(e1.id, 3, 1, TODAY); ok('over-capacity refused', false); } catch (err) { ok('over-capacity refused', err.status === 409 && err.message === 'Sorry, this event just filled up.'); }
  M.signUp(e1.id, 3, 0, TODAY);
  try { M.signUp(e1.id, 4, 0, TODAY); ok('full refused', false); } catch (err) { ok('full refused', err.status === 409); }
  try { M.updateSignup(e1.id, 1, 2, TODAY); ok('guest cap respected', false); } catch (err) { ok('guest cap respected', /Up to 1 guest/.test(err.message)); }
  try { M.updateSignup(e1.id, 2, 1, TODAY); ok('guest change beyond capacity refused', false); } catch (err) { ok('guest change beyond capacity refused', err.status === 409); }
  ...
  try { M.signUp(e2.id, 1, 0, TODAY); ok('past refused', false); } catch (err) { ok('past refused', err.status === 409); }
  ...
  try { M.createEvent({ name: 'X', event_date: '2026-12-01', league_id: 1, tournament_id: 1 }); ok('both links refused', false); } catch (err) { ok('both links refused', err.status === 400); }
  ...
  try { M.signUp(mo.id, 3, 0, TODAY); ok('a non-member cannot sign up', false, 'accepted'); } catch (err) { ok('a non-member cannot sign up', err.status === 403 && /club members/.test(err.message), err.message); }
