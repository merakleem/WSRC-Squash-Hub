...
// Run: node --test test/doubles-api.test.js
const bcrypt = require('bcryptjs');
const { suite, scratchDb } = require('./lib/suite');
const { boot, client } = require('./lib/client');

suite('the doubles ladder API', async ({ ok, t }) => {
  const app = boot(scratchDb(t, 'doubles-api'), ({ run }) => {
  ...
  const a = client(app), ann = client(app), ed = client(app);

  ok('everyone signs in', await a.login('', 'pw') === '302' && await ann.login('ann@x.invalid', 'pw123') === '302' && await ed.login('ed@x.invalid', 'pw123') === '302');

  console.log('\nRECORDING A DOUBLES MATCH');
  let r = await ann.send('POST', '/api/matches/doubles', { team1: [1, 2], team2: [3, 4], team1Score: 3, team2Score: 1 });
  ...
  r = await ed.send('POST', '/api/matches/doubles', { team1: [1, 2], team2: [3, 4], team1Score: 3, team2Score: 1 });
  ok('but not one they were not in', r.status === 403, String(r.status));
  r = await a.send('POST', '/api/matches/doubles', { team1: [1, 2], team2: [3, 5], team1Score: 1, team2Score: 3 });
  ok('an admin may record any', r.status === 200, JSON.stringify(r));
  r = await ann.send('POST', '/api/matches/doubles', { team1: [1, 2], team2: [2, 4], team1Score: 3, team2Score: 1 });
  ok('a player on both sides is refused', r.status === 400 && /different/.test(r.body?.error), JSON.stringify(r.body));
  r = await ann.send('POST', '/api/matches/doubles', { team1: [1, 2], team2: [3, 4], team1Score: 3, team2Score: 3 });
  ok('an impossible score is refused', r.status === 400 && /3 games/.test(r.body?.error), JSON.stringify(r.body));
  r = await ann.send('POST', '/api/matches/doubles', { team1: [1, 2], team2: [3, 4], team1Score: 3, team2Score: 0, playedOn: '2999-01-01' });
  ok('a future date is refused', r.status === 400 && /future/.test(r.body?.error), JSON.stringify(r.body));
  r = await ann.send('POST', '/api/matches/doubles', { team1: [1, 2], team2: [3, 99], team1Score: 3, team2Score: 0 });
  ok('an unknown player is refused', r.status === 400, JSON.stringify(r.body));

  console.log('\nTHE LADDERS');
  const dbl = await ann.get('/api/ladder/doubles/season');
  ...
  const hist = await ann.get('/api/players/1/history');
  ...
  console.log('\nTHE PROFILE PAYLOAD');
  const prof = await ann.get('/api/players/1/doubles');
  ...
  const bystander = await ann.get('/api/players/5/doubles');
  ...
  console.log('\nDELETING');
  r = await ann.send('DELETE', `/api/matches/doubles/${matchId}`);
  ok('a player cannot delete', r.status === 403, String(r.status));
  r = await a.send('DELETE', `/api/matches/doubles/${matchId}`);
  ok('an admin can', r.status === 200);
  ok('and the ladder shrinks to the remaining match', (await a.get('/api/ladder/doubles/season')).rows.length === 4);
});
