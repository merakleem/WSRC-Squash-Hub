...
// marker; an admin viewing as a member sees that member's, but must not spend
// their unread state for them, so nothing is written in that session.
function unreadFor(session) {
  ...
  const opened = _stamps(session.playerId, { write: !session.viewingAs });
  return {
    unread: {
