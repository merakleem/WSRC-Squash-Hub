 ...
 *
 * A single cutover season flips the club from the positional ladder to ratings;
 * everything from that season onward is rated. Comparing start years keeps this
 * correct whatever the labels look like.
 */
function ladderSystemFor(key, settings = {}) {
  const cutover = String(settings.elo_start_season || '').trim();
  if (!cutover) return 'leapfrog';
  const seasonYear = seasonStartYear(key);
  const cutoverYear = seasonStartYear(cutover);
  if (seasonYear == null || cutoverYear == null) return 'leapfrog';
  return seasonYear >= cutoverYear ? 'elo' : 'leapfrog';
}
