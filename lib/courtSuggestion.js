...
//      next two weeks), the earliest free start from now on the lowest-numbered
//      court that has one - today, then tomorrow.
//
...
/**
 * The earliest free start on the lowest-numbered court, today then tomorrow.
 * Courts are tried in their own order, not the clock's: a slot on court 1 wins
 * over an earlier one on court 3, which is how members pick for themselves.
 */
  ...
  for (const [i, date] of [today, _shiftDate(today, 1)].entries()) {
    ...
    for (const court of courts) {
      for (let start = floor; start + SLOT_MIN <= DAY_END; start += SLOT_MIN) {
        if (_isFree(spans, court.id, start, SLOT_MIN)) {
          return { date, startTime: _toTime(start), courtId: court.id, durationMinutes: SLOT_MIN };
        }
      }
    }
  }
