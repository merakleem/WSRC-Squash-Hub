 ...
 */
async function sendMany(payloads) {
  ...
  for (const payload of withFiles) {
    const one = await sendEmail(payload);
...
module.exports = { sendEmail, sendBatch, isConfigured, appUrl, RESEND_FROM, REDIRECT_TO, _applyRedirect, sendMany };
